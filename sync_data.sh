...
# */5 * * * * /home/pi/record/sync_data.sh >/dev/nul
# --------------------------------------------------------------------
...
function do_gdrive_sync()
{
	...
	echo "STATUS  - Processing pictures JPG in $DIR/$SYNC_DIR and OpenALPR ....."
	...
	for file in "$DIR/$SYNC_DIR/"*.jpg; do
		...
		full_fn=${file##*/}
		# iegūst tikai datnes nosaukumu bez paplašinājuma un ceļa uz datni, ieraksta mainīgajā fn
		fn=${full_fn%.*}
